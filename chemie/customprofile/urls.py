...
urlpatterns = [
    ...
    path("profil/glemt-passord/", views.forgot_password, name="forgotpassword"),
    re_path(
        r"^profil/aktiver/(?P<code>[a-z0-9]{32})/?",
        views.activate_password,
        name="activate",
    ),
    path("profil/medlemmer/endre/<int:profile_id>/<int:duration>/",views.change_membership_status,name="membership",),
    path("profil/medlemmer/", views.view_memberships, name="memberships"),
    path("profil/medlemmer/<int:year>",views.view_memberships,name="memberships-grade",),
