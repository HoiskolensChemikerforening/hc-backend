...
class BaseEvent(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.allowed_grades_previous:
            new_grades = set(self.allowed_grades) -\
                set(self.allowed_grades_previous)
            if new_grades:
...
class BaseRegistration(models.Model):
    ...
    status = models.IntegerField(
        ...
        default=REGISTRATION_STATUS.INTERESTED
        )
    ...
    def __str__(self):
        return '{} - {} - {}'\
            .format(self.event, self.user.get_full_name(), self.status)
