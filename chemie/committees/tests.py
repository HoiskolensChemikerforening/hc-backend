...
from .factories import (
    LowPermissionPositionFactory,
    HighPermissionPositionFactory,
)
...
class PositionTest(TestCase):
    def setUp(self):
        self.usercount = 4
        self.users = [
            RandomUserFactory.create() for x in range(0, self.usercount)
        ]
        ...
        self.edit_committee = lambda c: reverse(
            "verv:edit_description", kwargs={"slug": c.slug}
        )
        self.edit_members = lambda c: reverse(
            "verv:edit_memberships", kwargs={"slug": c.slug}
        )
    ...
    def test_position_manage_users(self):
        ...
        self.assertEqual(
            self.users[1] in self.low_permission_position.users.all(), True
        )
        self.low_permission_position.users.remove(self.users[1])
        self.assertEqual(
            self.users[0] in self.low_permission_position.users.all(), False
        )

    def test_position_max_members(self):
        self.assertRaises(
            ValidationError,
            self.low_permission_position.users.add,
            self.users[0],
            self.users[1],
        )
    ...
    def test_position_can_manage_committee(self):
        self.login_user()
        response = self.no_perms_user.get(
            self.edit_committee(self.low_committee), follow=False
        )
        self.assertEqual(response.status_code, 302)

        response = self.no_perms_user.get(
            self.edit_members(self.low_committee), follow=False
        )
        self.assertEqual(response.status_code, 302)
        ...
        response = self.low_user.get(
            self.edit_committee(committee), follow=True
        )
    ...
    def test_position_denied_manage_other_committees(self):
        ...
        self.login_user()
        response = self.low_user.get(
            self.edit_committee(self.high_permission_position.committee),
            follow=True,
        )
        self.assertContains(response, "Manglende rettigheter!")
        response = self.low_user.get(
            self.edit_members(self.high_permission_position.committee),
            follow=True,
        )
        self.assertContains(response, "Manglende rettigheter!")
