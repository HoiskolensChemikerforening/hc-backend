...
def test_election_not_open(client, create_user):
    ...
    assert (
        '<a class="button" href="/elections/results">'
...
def test_vote_for_one_user(
    ...
    request = client.post(
        ...
        {"Stem": "Avgi stemme", "candidates": candidate.id},
    )
...
def test_vote_blank(
    ...
    user = create_user
    client.login(username=user.username, password="defaultpassword")
    client.post(reverse("elections:voting"), {"Blank": "Stem blankt"})
    for cand in candidates:
        cand.refresh_from_db()
        assert cand.votes is 0
    position.refresh_from_db()
...
def test_invalid_vote(
    ...
    assert "Du stemte på {} kandidater, og det skal velges {} kandidater".format(
        len(candidates), position.spots
