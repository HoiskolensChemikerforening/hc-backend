...
class FormNode(Node):
    ...
    def render(self, context):  # noqa D102
        ...
        with context.push(
            ...
            # include
            children = (
                node for node in self.nodelist if isinstance(node, IncludeNode)
            )
            for included_list in children:
                included = included_list.template.resolve(context)
                children = (
                    node
                    for node in included.nodelist
                    if isinstance(node, FormPartNode)
                )
                _render_parts(context, children)
...
class FormPartNode(Node):
    ...
    def render(self, context):  # noqa D102
        ...
        # child parts
        children = (
            node for node in self.nodelist if isinstance(node, FormPartNode)
        )
        _render_parts(context, children)
